...
import 'package:cw_monero/transaction_history.dart' as loki_transaction_history;
...
List<TransactionInfo> _getAllTransactions(dynamic _) => loki_transaction_history
    .getAllTransations()
    .map((row) => TransactionInfo.fromRow(row))
    .toList();

class LokiTransactionHistory extends TransactionHistory {
  LokiTransactionHistory()
  ...
  @override
  Future<int> count() async => loki_transaction_history.countOfTransactions();
  ...
  Future refresh() async {
    ...
    try {
      _isRefreshing = true;
      loki_transaction_history.refreshTransactions();
      _isRefreshing = false;
