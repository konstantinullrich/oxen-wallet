...
import 'package:cw_monero/wallet_manager.dart' as loki_wallet_manager;
...
class LokiWalletsManager extends WalletsManager {
  LokiWalletsManager({@required this.walletInfoSource});

  static const type = WalletType.loki;
  ...
  Future<Wallet> create(String name, String password, String language) async {
    try {
      ...
      await loki_wallet_manager.createWallet(path: path, password: password, language: language);

      final wallet = await LokiWallet.createdWallet(
  ...
  Future<Wallet> restoreFromSeed(
    ...
    try {
      ...
      await loki_wallet_manager.restoreFromSeed(
          path: path,
          password: password,
          seed: seed,
  ...
  Future<Wallet> restoreFromKeys(
    ...
    try {
      ...
      await loki_wallet_manager.restoreFromKeys(
          path: path,
          password: password,
          language: language,
          restoreHeight: restoreHeight,
  ...
  Future<Wallet> openWallet(String name, String password) async {
    try {
      final path = await pathForWallet(name: name);
      loki_wallet_manager.openWallet(path: path, password: password);
      final wallet = await LokiWallet.load(walletInfoSource, name, type);
      await wallet.updateInfo();
      return wallet;
    } catch (e) {
      print('LokiWalletsManager Error: $e');
  ...
  Future<bool> isWalletExit(String name) async {
    try {
      final path = await pathForWallet(name: name);
      return loki_wallet_manager.isWalletExist(path: path);
    } catch (e) {
      print('LokiWalletsManager Error: $e');
      rethrow;
    }
